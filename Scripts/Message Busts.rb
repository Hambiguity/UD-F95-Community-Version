...
#------------------------------------------------------------------------------#
  ...
  DISABLE_SWITCH = 5   # Turn swith ON to disable busts and show normal face
   
  BUST_Z = -101           # adds to z value of busts if needed. A negative number
  ...
  MAIN_CHARACTERS = [2,3,4,6,7,8,10,13,14,15]
...
class Window_Message < Window_Base
  ...
  def update_bust  
    if !$game_message.bust_name.empty? && !$game_switches[Galv_Bust::DISABLE_SWITCH]     
      ...
      if $game_switches[112] == true then
        Galv_Bust::MAIN_CHARACTERS.each do | actor_id |
          puts "COMPARING ACTOR NAME: " + $game_actors[actor_id].name.downcase + " to: " + $game_message.bust_name.downcase
            if $game_actors[actor_id].name.downcase == $game_message.bust_name.downcase then
              $game_message.bust_name += "-nude"
            end
            if $game_message.bust_name == "MsAmos" then
              $game_message.bust_name += "-nude"
            end
        end
      end
      
      @bust.bitmap = Cache.busts($game_message.bust_name + "-" + ($game_message.bust_index + 1).to_s)
      
      if $game_message.bust_name.start_with?($game_actors[1].name) then
          $game_message.mirror = false
      else
          $game_message.mirror = true
      end
