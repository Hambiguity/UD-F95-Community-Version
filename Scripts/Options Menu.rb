...
WINDOW_ORDER = [:master,:bgm,:se,:resolution,:message_se,:autosave]
#Basic options are:
...
class Scene_Options < Scene_MenuBase
  ...
  def resolution_ok
    ...
    when 1
      $game_options.set_resolution(960,620)
    when 2
      $game_options.set_resolution(640,480)
    when 3
...
class Window_OR < Window_Selectable
  TEXT = ["1280x960","960x620","640x480","Fullscreen"]
  def initialize(y)
